      ...
      <main id="main-content">
        ...
        <section className="relative pt-20">
          <div className="relative h-[50vh] md:h-[60vh] overflow-hidden">
            ...
            <div className="absolute inset-0 bg-gradient-to-r from-dark-primary/80 to-transparent" />
            ...
            <div className="absolute bottom-0 left-0 right-0 z-10">
              <div className="max-w-[1400px] mx-auto px-6 lg:px-12 pb-12 lg:pb-16">
                ...
                <div className="max-w-3xl">
                  <p className="text-xs text-accent-light uppercase tracking-[0.2em] font-medium mb-4">{service.title}</p>
                  <h1 className="font-heading text-4xl md:text-5xl lg:text-6xl font-bold text-white leading-[1.1] mb-6">
                    {service.tagline}
                  </h1>
                  <p className="text-lg text-white/60 leading-relaxed max-w-2xl mb-8">{service.description}</p>
                  <div className="flex gap-4">
                    ...
                    <Link href="/work" className="inline-flex items-center justify-center font-medium rounded-md border border-white/25 text-white/80 hover:text-white hover:border-white/50 px-8 py-3.5 text-sm uppercase tracking-widest transition-all duration-300">
                      See Results
                    </Link>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 divide-x divide-card-light-border">
              {service.metrics.map((m) => (
                <div key={m.label} className="py-8 px-6 text-center">
                  <div className="font-heading text-2xl md:text-3xl font-bold text-text-dark mb-1">{m.value}</div>
                  <div className="text-[11px] text-text-dark-muted uppercase tracking-widest">{m.label}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-24 lg:py-32 bg-dark-primary">
          <div className="max-w-[1400px] mx-auto px-6 lg:px-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
              <ScrollReveal>
                <p className="text-xs text-accent-light uppercase tracking-[0.2em] font-medium mb-4">Technology</p>
                <h3 className="font-heading text-2xl font-bold text-text-light mb-8">Stack & Tools</h3>
                <div className="flex flex-wrap gap-2">
                  {service.techStack.map((tech) => (
                    <span key={tech} className="px-4 py-2 border border-card-dark-border text-text-light-muted text-sm font-medium bg-card-dark">
                      {tech}
                    </span>
                  ))}
                </div>
              ...
              <ScrollReveal delay={0.15}>
                <p className="text-xs text-accent-light uppercase tracking-[0.2em] font-medium mb-4">Business Outcomes</p>
                <h3 className="font-heading text-2xl font-bold text-text-light mb-8">What You Get</h3>
                <div className="space-y-4">
                  {service.benefits.map((b) => (
                    <div key={b} className="flex items-start gap-3">
                      <CheckCircle2 size={14} className="text-accent-light flex-shrink-0 mt-1" />
                      <span className="text-text-light-muted leading-relaxed">{b}</span>
                    </div>
                  ))}
                </div>
              </ScrollReveal>
        ...
        <section className="py-24 lg:py-32 bg-light-primary">
          <div className="max-w-[1400px] mx-auto px-6 lg:px-8">
            <ScrollReveal>
              <p className="text-xs text-accent uppercase tracking-[0.2em] font-medium mb-4">Case Study</p>
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 border border-card-light-border bg-white overflow-hidden">
                ...
                <div className="lg:col-span-8 p-8 lg:p-12">
                  <h3 className="font-heading text-2xl font-bold text-text-dark mb-6">{service.caseStudy.title}</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                    <div>
                      <p className="text-[11px] text-accent uppercase tracking-wider font-semibold mb-2">The Challenge</p>
                      <p className="text-text-dark-muted text-sm leading-relaxed">{service.caseStudy.challenge}</p>
                    </div>
                    <div>
                      <p className="text-[11px] text-accent uppercase tracking-wider font-semibold mb-2">The Result</p>
                      <p className="text-text-dark-muted text-sm leading-relaxed">{service.caseStudy.result}</p>
                    </div>
                  </div>
                  <div className="flex items-center justify-between pt-6 border-t border-card-light-border">
                    <div className="flex items-center gap-2">
                      <span className="h-2 w-2 rounded-full bg-accent" />
                      <span className="font-heading text-lg font-bold text-accent">{service.caseStudy.metric}</span>
                    </div>
                    <Link href="/work" className="text-sm font-semibold text-accent flex items-center gap-1.5 hover:gap-2.5 transition-all">

      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[60] bg-dark-primary/98 glass-effect lg:hidden"
          >
            <div className="absolute top-4 right-4">
              <button
                onClick={() => setMobileOpen(false)}
                className="text-text-light p-2 cursor-pointer"
            ...
            <div className="flex flex-col items-center justify-center h-full gap-6">
              {navLinks.map((link, i) => (
                <motion.a
                  ...
                  onClick={() => setMobileOpen(false)}
                  className="font-heading text-xl text-text-light hover:text-accent transition-colors tracking-wider"
                >
                  {link.label}
                </motion.a>
              ))}
